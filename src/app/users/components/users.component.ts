...
export class UsersComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.organizationApolloService.getOrganizationUsers().pipe(first()).subscribe((users) => {
      users.forEach(user => {
        user.avatarUri = this.getAvatarUri(user)
        if (user.role == "ENGINEER") {
          this.engineers.push(user);
    ...
    this.organizationApolloService.getUserInfo().pipe(first())
      .subscribe((user) => {
        this.avatarUri = this.getAvatarUri(user)
        if (this.organizationInactive) {
